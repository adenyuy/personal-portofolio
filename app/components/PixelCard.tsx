...
class Pixel {
  ...
  constructor(
    ...
    // Use a small maxSize so the pixels remain as dots that don't obscure the photo
    this.maxSizeInteger = 2.5; 
    this.maxSize = this.getRandomValue(1, this.maxSizeInteger);
  ...
  draw() {
    // Center the pixel drawing relative to its grid cell
...
const VARIANTS = {
  ...
  orange: {
    activeColor: '#fed7aa',
    gap: 5, // Back to smaller gap
    speed: 60,
    colors: '#fed7aa,#fb923c,#e8481a,#9a3412',
    noFocus: true,
  },
};
...
export default function PixelCard({
  ...
  const initPixels = () => {
    ...
    // USE CLIENT WIDTH/HEIGHT to ignore CSS scale transforms (which getBoundingClientRect includes)
  ...
  return (
    ...
    >
      {/* Pixel canvas sits ON TOP of children */}
      <canvas
        ref={canvasRef}
